...
EKA-AI uses **PyPA Trusted Publishing** (OpenID Connect) — no PyPI API tokens stored in secrets.

### One-time setup

1. **Create PyPI environments** in your GitHub repository settings:
   - `pypi` (production)
   - `testpypi` (staging)

2. **Configure Trusted Publishing** on PyPI / TestPyPI:
   - PyPI → Account → Publishing → Add a new pending publisher
   - Set: owner = `eka-ai`, repository = `eka-ai`, workflow = `publish.yml`, environment = `pypi`

3. **Set environment protection rules** (optional but recommended):
   - Require a reviewer before deploying to `pypi`

### Release workflow

```bash
# 1. Bump version in pyproject.toml and eka_ai/__init__.py
...
#    b. Build wheel + sdist
#    c. Publish to PyPI via Trusted Publishing
```

### Test on TestPyPI first

```bash
# Trigger manually from GitHub Actions → publish.yml → Run workflow
# Select environment: testpypi

# Install from TestPyPI to verify
pip install -i https://test.pypi.org/simple/ eka-ai==1.0.1
```
...
Model files are hosted on Google Drive and cached locally. The downloader is transparent — you never need to call it manually.

```python
# Manual control (if needed)
from eka_ai.downloader import download_all, get_cache_dir

paths = download_all()          # download both files
print(get_cache_dir())          # ~/.cache/eka_ai
...
# Custom cache dir
import os
os.environ["EKA_CACHE_DIR"] = "/path/to/my/cache"
```
